...
namespace Practic7
{
    static class Base
    {
        ...
        public static void Base2(ConsoleKey key, int position, int max, int min, string path)
        {
            ...
            do
            {
                Switch.WriteCursor(position);
                key = Console.ReadKey().Key;
                position = Switch.CursorPosition(position, max, min, key);
                    ...
                    if (directory.Exists)
                    {
                        ...
                        do
                        {
                            ...
                            else if (key == ConsoleKey.Escape)
                            {
                                ...
                                min = 1;
                                max = 2;
                                Base2(key, position, max, min, path);
                                check = false;
                            }
                        } while (check);
                    }
